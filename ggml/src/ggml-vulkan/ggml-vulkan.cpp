...
static void ggml_vk_mul_mat_q_f16(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    const uint32_t split_k = ggml_vk_guess_split_k(ctx, ne01, ne11, ne10, false, pipeline);
    ...
    {
        if (do_tiling) {
            if (ctx->prealloc_size_tile < ctx->device->tiling_threshold) {
...
static void ggml_vk_mul_mat(ggml_backend_vk_context * ctx, vk_context& subctx, const struct ggml_cgraph * cgraph, int node_idx) {
    ...
    if (needs_split) {
        ...
        while (m_offset < dst->ne[0]) {
            ...
            ggml_vk_mul_mat_q_f16(ctx, subctx, &src02, src1, &dst2);
...
static void ggml_vk_op_f32(ggml_backend_vk_context * ctx, vk_context& subctx, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * src3, ggml_tensor * dst, ggml_op op, PC&& pc) {
    ...
    if (op == GGML_OP_OUT_PROD) {
        ...
        if (do_tiling) {
            ...
            // Request additional descriptor sets for tiling (1 already requested above)
            if (num_dispatches > 1) {
                ggml_pipeline_request_descriptor_sets(ctx, pipeline, num_dispatches - 1);
            }

            // Ensure tile buffer is allocated
            ctx->prealloc_size_tile = ctx->device->tiling_threshold;
...
#endif // GGML_VULKAN_CHECK_RESULTS
...
static void ggml_vk_check_results_0(ggml_backend_vk_context * ctx, ggml_cgraph * cgraph, int tensor_idx) {
    ...
    int rms_norm_idx = -1;
    if (ctx->num_additional_fused_ops == 1 &&
        cgraph->nodes[tensor_idx]->op == GGML_OP_RMS_NORM &&
    ...
    // Ensure all previous GPU compute work is complete before reading buffers.
    // On UMA devices, ggml_vk_buffer_read uses direct memcpy from mapped memory
    // without GPU synchronization, so we need an explicit wait here.
